...
- **🚨 BUILT-IN TOOL SEAT SSOT = `TOOL_SEAT` in `packages/shared/src/prompts/generation-policy.ts` (added 2026-09-21).** Which model the character sheet, the establishing image, the grid extract and the image edit render on when the caller names none. `'default-image'` FOLLOWS `defaultModelFor('image')`, so promoting a better image model moves those tools with nothing else edited; a model id pins a tool, and `model-facts.ts` throws at load for a pinned id that is not a live image model. `toolModelFor(tool)` resolves it here, for the character op's `model` description and the generated `sheet-tool-defaults` partial injected into `slates-character-identity.md`. The desktop resolves the SAME table through its generated mirror (`slate/scripts/sync-generation-policy.mjs` → `toolModel` in `slate/src/shared/pricing.ts`), so the handler that bills, the screens that print the price and the text an agent reads cannot name different models. The sheet framing and GPT tier (`SHEET_TOOL_ASPECT_RATIO`, `SHEET_TOOL_GPT_QUALITY`) live beside it. **Why:** the op description said "default (nano-banana-2)" and the skill said "Nano Banana 2 at 2K", both typed, while the desktop typed the same id at every use and three of its screens priced a sheet from their own copy. **Never type a tool's model into an op description or a skill**, and keep `generation-policy.ts` a dependency-free leaf, because it is copied verbatim into the desktop. After editing it: `node scripts/sync-generation-policy.mjs` in slate, then `npm run render-partials -w packages/shared`, `npm run sync-partials -w packages/shared` and `npm run sync-prompt-builder` here.
...
- **Audio MIN / MAX / DEFAULT seconds live in THREE repos — a change is a three-site edit.** Here (`SEED_AUDIO_MIN_SECONDS` &co, feeding `audioCostKey`), `slate/src/shared/pricing.ts` (MODEL_REGISTRY `audio.durationSeconds`, feeding `clampAudioDuration` + `audioCreditKey`), and `slates-api/src/lib/audio-keys.ts` (the server's fail-closed bounds). `audioCostKey` must **mirror the desktop's clamping exactly, non-finite arm included** — `Math.max(min, NaN)` is NaN, so a missing arm yields the key `seed-audio-NaNs` while the desktop quotes the default. The MINs were absent and the floor was a hardcoded 1, which quoted a real `seed-audio-2s` price for a generation the proxy REJECTS. `pricing-consistency-check.mjs` §4 now sweeps **out-of-range values deliberately** — the property under test is not "is it in range" but "do the two clamp IDENTICALLY", so skipping the values where they can differ skips the whole test.
  - **The TTS seat's character bucket is the same three-site shape (added 2026-09-05).** `TTS_BUCKET_CHARS` (250) and the 2,000-character cap — read from `MODEL_CAPABILITIES['inworld-tts-2'].maxCharacters` here and in `slate/src/shared/pricing.ts`, hand-typed as the server's fail-closed bound in `slates-api/src/lib/audio-keys.ts` — with `clampTtsCharacters` mirrored in both clients, non-finite arm included (NaN quotes the first bucket on both sides rather than `-NaNc` on one). Keys end in `c`, never `s`, so a TTS key cannot pass a duration gate. `pricing-consistency-check.mjs` §2d sweeps every count 1..2000 and asserts the server refuses 2,001; §4 asserts the two clamps agree. The cloning spec (`voiceClone`: clip bounds, design-prompt bounds, the measured 2-a-minute workspace-wide clone ceiling) lives on the same capability row and is read by the op schema (`TTS_VOICE_CLONE`) and the desktop's voice picker — never retyped into a form control.
...
- **🚨 THE PROTOCOL IS THE PRODUCT SURFACE.** `packages/mcp/src/server.ts` used exactly one MCP
  ...
  every bundled skill as a **prompt**, the manual / capability table / live price list as **resources**,
