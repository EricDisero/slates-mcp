...
export const getChatGptStatus: Operation<Record<string, never>> = {
...
export const connectChatGpt: Operation<Record<string, never>> = {
...
export const saveExternalImage: Operation<{
  projectId: string
  filePath?: string
  dataUrl?: string
  assetId?: string
  ...
  requestedSettings?: Record<string, string | number | boolean>
}> = {
  ...
  async run(input, ctx) {
    ...
    const result = await desktop.post<{ asset: unknown }>('/agent/assets/external-image', {
      ...input,
      assetId: input.assetId ? refs.get(input.assetId)!.id : undefined,
      referenceAssetIds: (input.referenceAssetIds ?? []).map(id => refs.get(id)!.id),
    })
    return ok(result, JSON.stringify(result) + '\n' + describeResolvedRefs(
      (input.referenceAssetIds ?? []).map(ref => ({ ref, role: 'reference image' })), refs,
    ))
  },
}
...
export const ALL_OPERATIONS: ReadonlyArray<Operation<unknown>> = [
  ...
  generateChatGptImage as unknown as Operation<unknown>,
