...
export class SlatesCloudClient {
  ...
  private async fetchOrFriendly(path: string, url: string, init: RequestInit): Promise<Response> {
    try {
      ...
      if (name === 'TimeoutError' || name === 'AbortError') {
        throw new Error(
          `slates-api ${path} timed out. The request may still have been received — ` +
